    ...
    <div class="icard">
      ...
      <div v-else>
        <div v-for="a in appointments" :key="a.id" style="padding:14px 18px;border-bottom:1px solid var(--cloud);cursor:pointer" @click="$router.push({ name: 'student-appointment-show', params: { id: a.id } })">
          <div v-if="a.request_status === 'awaiting_student' && a.status !== 'cancelled'" style="font-size:13.5px;font-weight:600;color:var(--amber)">Awaiting Your Schedule Selection</div>
          ...
          <div style="font-size:12px;color:var(--stone);margin-top:2px">{{ a.unit }}</div>

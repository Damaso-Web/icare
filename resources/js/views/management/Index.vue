<template>
  <div class="fade-up">
    <div class="ph" style="margin-bottom:20px">
      <h1>Management</h1>
      <p>Manage Colleges, Programs, Departments, and referral form options used across iCARE.</p>
    </div>
    ...
    <!-- ============ REFERRAL FORMS ============ -->
    <div v-if="activeTab === 'forms'" class="icard">
      <div class="icard-header" style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
        <span class="icard-title">Referral Form Options</span>
        <div style="display:flex;gap:8px;align-items:center">
          <select v-model="formCategory" class="fsm" @change="fetchFormOptions">
            <option value="referral_type">Service Requested</option>
            <option value="referral_source">Referral Source</option>
            <option value="act_of_misconduct">Act of Misconduct</option>
          </select>
      ...
      <div style="padding:12px 18px;font-size:12px;color:var(--fog);border-bottom:1px solid var(--cloud)">
        These populate the dropdowns on the Refer Student form. Editing a Label only changes how it displays - existing referrals keep their stored value.
      </div>
      <div v-if="loadingFormOptions" style="text-align:center;padding:44px">
        <div style="width:24px;height:24px;border:2px solid var(--mint);border-top-color:var(--moss);border-radius:50%;animation:spin .7s linear infinite;margin:0 auto"></div>
      </div>
      <div v-else-if="formOptions.length === 0" class="empty-state">
      ...
      <div class="ts" v-else>
        <table class="itable">
          <thead>
            <tr><th>ID</th><th>Label</th><th>Stored Value</th><th></th></tr>
          </thead>
          <tbody>
            <tr v-for="o in formOptions" :key="o.id">
              <td style="font-family:var(--mono);font-size:12px">{{ o.id }}</td>
              <td>{{ o.label }}</td>
              <td style="font-family:var(--mono);font-size:12px;color:var(--fog)">{{ o.value }}</td>
              <td>
                <div style="display:flex;gap:6px;justify-content:flex-end">
                  <button class="ibtn ibtn-o ibtn-sm" @click="openFormOptionModal(o)">Edit</button>
                  <button class="ibtn ibtn-sm" style="background:var(--red-lt);color:var(--red);border:1.5px solid #f5c0c0" @click="deleteFormOption(o)">Delete</button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <div v-if="showFormOptionModal" style="position:fixed;inset:0;background:rgba(0,0,0,.42);z-index:60;display:flex;align-items:center;justify-content:center;padding:20px" @click.self="showFormOptionModal = false">
      <div style="background:#fff;border-radius:var(--r-lg);width:100%;max-width:420px;overflow:hidden;box-shadow:var(--sh-lg)">
        <div style="padding:20px 22px;border-bottom:1px solid var(--cloud)">
          <div style="font-size:15px;font-weight:600;color:var(--ink)">{{ formOptionForm.id ? 'Edit Option' : 'Add Option' }}</div>
        </div>
        <div style="padding:22px;display:flex;flex-direction:column;gap:14px">
          ...
          <div v-if="!formOptionForm.id">
            ...
            />
            <div style="font-size:11px;color:var(--stone);margin-top:4px">A short internal code (letters, numbers, underscores only). This gets stored on referral records and can't be changed later.</div>
          </div>
          <div v-else>
            ...
            <div style="font-size:11px;color:var(--stone);margin-top:4px">Not editable - existing referrals reference this value.</div>
          </div>
          <div style="display:flex;gap:8px">
            <button class="ibtn ibtn-p" :disabled="!formOptionForm.label.trim() || (!formOptionForm.id && !formOptionForm.value.trim()) || savingFormOption" @click="saveFormOption">
              {{ savingFormOption ? 'Saving...' : (formOptionForm.id ? 'Save Changes' : 'Add Option') }}
            </button>
...
const tabs = [
  ...
  { key: 'forms',       label: 'Referral Forms' },
];
...
const formCategory = ref('referral_type');
const showFormOptionModal = ref(false);
const formOptionForm = ref({ id: null, category: 'referral_type', value: '', label: '' });
...
async function fetchFormOptions() {
  ...
  try {
    const res = await axios.get(`${API_BASE}/management/form-options`, { ...authHeaders(), params: { category: formCategory.value } });
...
function openFormOptionModal(o) {
  formOptionForm.value = o
    ? { id: o.id, category: o.category, value: o.value, label: o.label }
    : { id: null, category: formCategory.value, value: '', label: '' };
...
async function saveFormOption() {
  ...
  try {
    if (formOptionForm.value.id) {
      await axios.put(`${API_BASE}/management/form-options/${formOptionForm.value.id}`, { label: formOptionForm.value.label }, authHeaders());
    } else {
    ...
    formOptionError.value = e.response?.data?.message || 'Failed to save option.';
  } finally {
